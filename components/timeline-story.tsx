    ...
    <article className={`${standalone ? 'event-story standalone-story' : 'event-story'} ${event.layout === 'editorial' ? 'editorial-story' : ''}`}>
      ...
      <div className="detail-body">
        ...
        {event.content?.map((block, index) => {
          ...
          if (block.type === 'imageAside') return <section className="article-image-aside" key={index}><div><h2>{block.heading}</h2>{block.image.caption && <p className="article-image-description"><InlineMarkup text={block.image.caption} /></p>}{block.paragraphs.map((text, i) => <p key={i}><InlineMarkup text={text} /></p>)}</div><figure><img src={block.image.src} alt={block.image.alt} loading="lazy" /></figure></section>;
